...
fetchChannelConfig() {
  ...
  set -x
  docker run smartbft/fabric-tools configtxlator proto_decode --input config_block.pb --type common.Block | jq .data.data[0].payload.data.config >"${OUTPUT}"
  set +x
}
...
createConfigUpdate() {
  ...
  set -x
  docker run smartbft/fabric-tools configtxlator proto_encode --input "${ORIGINAL}" --type common.Config >original_config.pb
  docker run smartbft/fabric-tools configtxlator proto_encode --input "${MODIFIED}" --type common.Config >modified_config.pb
  docker run smartbft/fabric-tools configtxlator compute_update --channel_id "${CHANNEL}" --original original_config.pb --updated modified_config.pb >config_update.pb
  docker run smartbft/fabric-tools configtxlator proto_decode --input config_update.pb --type common.ConfigUpdate >config_update.json
  echo '{"payload":{"header":{"channel_header":{"channel_id":"'$CHANNEL'", "type":2}},"data":{"config_update":'$(cat config_update.json)'}}}' | jq . >config_update_in_envelope.json
  docker run smartbft/fabric-tools configtxlator proto_encode --input config_update_in_envelope.json --type common.Envelope >"${OUTPUT}"
  set +x
}
