...
public:
    ...
    {
	PBFT_LOG(DEBUG)<<"We try to encode a base message!";
        PBFT_LOG(DEBUG)<<"The version is "<<m_baseMessage->version();
        PBFT_LOG(DEBUG)<<"The index is "<<m_baseMessage->index();
        PBFT_LOG(DEBUG)<<"The hash is "<<m_baseMessage->hash();
        PBFT_LOG(DEBUG)<<"The view is "<<m_baseMessage->view();
        PBFT_LOG(DEBUG)<<"The timestamp is "<<m_baseMessage->timestamp();
        PBFT_LOG(DEBUG)<<"The generatedFrom is "<<m_baseMessage->generatedfrom();
        PBFT_LOG(DEBUG)<<"The signatureHash is "<<m_baseMessage->signaturehash();
        PBFT_LOG(DEBUG)<<"The signatureData is "<<m_baseMessage->signaturedata(); 
        PBFT_LOG(DEBUG)<<"The encode data is "<<bcos::protocol::encodePBObject(m_baseMessage);
