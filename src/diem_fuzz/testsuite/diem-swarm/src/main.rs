...
use diem_swarm::{client, faucet, swarm::{DiemSwarm,DiemNode}};
...
struct Args {
    ...
    pub num_nodes: usize,
    /// Number of fuzzer nodes to start (0 by default)
    #[structopt(short = "t", long, default_value = "0")]
    pub num_fuzzer_nodes: usize,

    // Number of cov nodes to start(0 by default)
    //#[structopt(short = "v", long, default_value = "0")]
    //pub num_cov_nodes: usize,
    /// Start client
    ...
    /// Path to the fuzzer-node binary
    #[structopt(long)]
    pub fuzzer_node: String,

    // Path to the cov-node binary
    // #[structopt(long)]
    // pub cov_node: String,
...
fn main() {
    ...
    let num_fuzzer_nodes = args.num_fuzzer_nodes;
    //let num_cov_nodes = args.num_cov_nodes;
    ...
    let mut validator_swarm = DiemSwarm::configure_validator_swarm(
        ...
        args.fuzzer_node.as_ref(),
        // if calculate the coverage, this should be cov_node
        args.diem_node.as_ref(),
        num_nodes,
    ...
    let num_cov_nodes = 0;
    validator_swarm
        .launch_attempt(true,num_fuzzer_nodes,num_cov_nodes)
    ...
    if args.start_client {
        ...
        std::thread::spawn(move ||{
           random_stop_nodes(validator_swarm.nodes(),num_fuzzer_nodes);
        });
...
// // add by fcorleone, for crusader fuzzing
pub fn random_stop_nodes(nodes: &mut HashMap<String, DiemNode>, fuzzer_num: usize){
    ::std::thread::sleep(::std::time::Duration::from_secs(1500));
    loop{
        for (nodeid, node) in &mut *nodes{
            let nodeid_int : u64 = nodeid.parse::<u64>().unwrap();
            // println!("current node id is {}", nodeid_int);
            // println!("current fuzzer number is {}",fuzzer_num);
            if nodeid_int >= (fuzzer_num as u64) {
                continue;
            }
            else{
                // node.process.unwrap().drop();
                node.stop();
                // std::fs::write("~/fuzzer_log", "node has stopped!!!").unwrap();
                println!("node {} has stopped!!!",nodeid);
                ::std::thread::sleep(::std::time::Duration::from_secs(60));
                node.start().unwrap();
                // std::fs::write("~/fuzzer_log", "node has startted!!!").unwrap();
                println!("node {} has started!!!",nodeid);
