...
impl RoundManager {
    ...
    async fn process_new_round_event(
        ...
        {
            info!("i'm the proposer!!");
            // let proposal_msg_future =
            // ConsensusMsg::ProposalMsg(Box::new(self.crusader_generate_proposal_future(new_round_event.round + 3).await?));
            // let proposal_msg_before =
            // ConsensusMsg::ProposalMsg(Box::new(self.crusader_generate_proposal_before(1).await?));
            // let proposal_msg =
            //     ConsensusMsg::ProposalMsg(Box::new(self.generate_proposal(new_round_event).await?));
            let mut network = self.network.clone();
            // crusader fuzzer changed this to send to random nodes
            // network.broadcast(proposal_msg).await;
            let mut proposal_msg = mutate_proposal(self.generate_proposal(new_round_event).await?,self.safety_rules.clone(),self.proposal_generator.author());
            network.send_to_others(ConsensusMsg::ProposalMsg(Box::new(proposal_msg)));
            // network.send_to_others(proposal_msg_future);
            // network.send_to_others(proposal_msg_before);
            // network.broadcast(proposal_msg_future);
            // network.broadcast(proposal_msg_before);
    ...
    async fn crusader_generate_proposal_future(
        &mut self,
        future_round:u64,
    ) -> anyhow::Result<ProposalMsg> {
        // Proposal generator will ensure that at most one proposal is generated per round
        let proposal = self
            .proposal_generator
            .generate_proposal(future_round)
            .await?;
        let signature = self.safety_rules.lock().sign_proposal(&proposal)?;
        let signed_proposal =
            Block::new_proposal_from_block_data_and_signature(proposal, signature);
    ...
    async fn crusader_generate_proposal_before(
        &mut self,
        before_round: u64,
    ) -> anyhow::Result<ProposalMsg> {
        // Proposal generator will ensure that at most one proposal is generated per round
        let proposal = self
            .proposal_generator
            .generate_proposal(before_round)
            .await?;
        let signature = self.safety_rules.lock().sign_proposal(&proposal)?;
        let signed_proposal =
            Block::new_proposal_from_block_data_and_signature(proposal, signature);
    ...
    async fn sync_up(
        ...
        if help_remote && local_sync_info.has_newer_certificates(&sync_info) {
            ...
            //self.network.send_sync_info(local_sync_info.clone(), author);
             self.network.send_to_others(ConsensusMsg::SyncInfo(Box::new(local_sync_info.clone())));
        }
...
// Crusader:: here need to fuzz!!
        ...
        // Crusader:: here need to fuzz!! We have changed this to broadcast and delete await;
        self.network.send_to_others(timeout_vote_msg);
        // self.network.broadcast(timeout_vote_msg);
        error!(
            round = round,
    ...
    async fn process_proposal(&mut self, proposal: Block) -> Result<()> {
        ...
        let mut new_vote_msg = mutate_vote(vote_msg.clone());
        // self.network.send_vote(vote_msg, vec![recipients]).await;
        self.network.send_to_others(ConsensusMsg::VoteMsg(Box::new(new_vote_msg)));
        Ok(())
    }
