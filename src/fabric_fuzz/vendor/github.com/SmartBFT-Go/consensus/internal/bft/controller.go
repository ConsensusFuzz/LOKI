...
func (c *Controller) ProcessMessages(sender uint64, m *protos.Message) {
	...
	//c.Logger.Debugf("%d got message from %d", c.ID, sender)
...
//BroadcastConsensus broadcasts the message for fuzz
func (c *Controller) BroadcastConsensusFuzz(m *protos.Message) {
	for _, node := range c.NodesList {
		// Do not send to yourself
		if c.ID == node {
			continue
		...
		//c.Comm.SendConsensus(node, m)
		c.Comm.SendConsensusFuzz(node, m)
	}
