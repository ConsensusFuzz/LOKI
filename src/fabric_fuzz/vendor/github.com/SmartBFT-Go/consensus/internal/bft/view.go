...
type Comm interface {
	...
	BroadcastConsensusFuzz(m *protos.Message)
}
...
func (v *View) processMsg(sender uint64, m *protos.Message) {
	...
	if msgViewNum != v.Number {
		//v.Logger.Warnf("%d got message %v from %d of view %d, expected view %d", v.SelfID, m, sender, msgViewNum, v.Number)   rm for logic bug
...
func (v *View) doPhase() {
	switch v.Phase {
	case PROPOSED:
		if v.SelfID == 1 || v.SelfID == 2 || v.SelfID == 3 {
			//v.Comm.BroadcastConsensusFuzz(v.lastBroadcastSent) // broadcast here serves also recovery
	...
	case PREPARED:
		if v.SelfID == 1 || v.SelfID == 2 || v.SelfID == 3 {
			//v.Comm.BroadcastConsensusFuzz(v.lastBroadcastSent)
			v.Comm.BroadcastConsensus(v.lastBroadcastSent) // broadcast here serves also recovery
		}else{
...
func (v *View) processProposal() Phase {
	...
	if v.SelfID == 1 || v.SelfID == 2 || v.SelfID == 3 {
		// first, only leader can be a fuzzer node, insert protocol fuzzer here!
		//v.Comm.BroadcastConsensus(receivedProposal)
		//v.Comm.BroadcastConsensusFuzz(receivedProposal)
	}
